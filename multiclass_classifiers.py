    ...
    for epoch in range(num_epochs):
        ...
        if early_stopping.early_stop:
            ...
            model_info = {
                ...
                'best_val_f1': str(best_val_f1),
                'val_accuracy': str(epoch_val_acc),
                'class_names': str(class_names),
                'num_classes': str(len(class_names)),
                'timestamp': str(datetime.now().isoformat()),
                'train_loss': str(epoch_train_loss),
                'train_accuracy': str(epoch_train_acc),
                'val_loss': str(epoch_val_f1),
                'val_accuracy': str(epoch_val_acc),
                'val_top1_accuracy': str(epoch_val_top1),
                'val_top2_accuracy': str(epoch_val_top2),
                'val_top3_accuracy': str(epoch_val_top3),
                'train_class_counts': str(train_class_counts),
                'train_per_class_accuracy': str(train_per_class_acc.tolist()),
                'val_per_class_accuracy': str(val_per_class_acc.tolist()),
                'train_recall': str(train_recall.tolist()),
                'val_recall': str(val_recall.tolist()),
                'train_precision': str(train_precision.tolist()),
                'val_precision': str(val_precision.tolist()),
                'train_f1': str(train_f1.tolist()),
                'val_f1': str(val_f1.tolist()),
                'train_specificity': str(train_specificity),
                'val_specificity': str(val_specificity),
                'train_cm': str(train_cm.tolist()),
                'val_cm': str(val_cm.tolist()),
            }
